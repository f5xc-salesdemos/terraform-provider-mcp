...
export class OperationsService {
  ...
  constructor(basePath?: string) {
    ...
    const possiblePaths = basePath
      ? [path.join(basePath, 'metadata', 'operations-metadata.json')]
      : dataDir
        ? [path.join(dataDir, 'metadata', 'operations-metadata.json')]
        : [path.join(packageRoot, 'dist', 'metadata', 'operations-metadata.json')];
