...
export const SearchDocsSchema = z
  .object({
    ...
    type: z.enum(['resource', 'data-source', 'function', 'guide']).optional().describe('Filter by documentation type'),
...
export const GetDocSchema = z
  .object({
    ...
    type: z
      .enum(['resource', 'data-source', 'function', 'guide'])
      .optional()
...
export const SearchApiSpecsSchema = z
  .object({
    query: z
      .string()
      .min(1, 'Query is required')
...
// Get API spec schema
export const GetApiSpecSchema = z
  .object({
    name: z
      .string()
      .min(1, 'Spec name is required')
      .describe('Name of the API specification (e.g., "http_loadbalancer", "app_firewall", "namespace")'),
    include_paths: z.boolean().default(true).describe('Whether to include endpoint paths in the response'),
    include_definitions: z.boolean().default(false).describe('Whether to include schema definitions (can be large)'),
    response_format: ResponseFormatSchema,
  })
  .strict();

// Find endpoints schema
export const FindEndpointsSchema = z
  .object({
    pattern: z
      .string()
...
export const GetSchemaDefSchema = z
  .object({
    spec_name: z
      .string()
      .min(1, 'Spec name is required')
...
export const ListDefinitionsSchema = z
  .object({
    spec_name: z.string().min(1, 'Spec name is required').describe('Name of the API specification'),
    response_format: ResponseFormatSchema,
  })
  .strict();
...
export const GetSubscriptionInfoSchema = z
  .object({
    resource: z.string().optional().describe('Resource name to check (omit to get all Advanced tier resources)'),
    tier: z.enum(['STANDARD', 'ADVANCED']).optional().describe('Filter by subscription tier'),
    response_format: ResponseFormatSchema,
  })
  .strict();

// Get property subscription info schema
export const GetPropertySubscriptionInfoSchema = z
  .object({
    resource: z
      .string()
      .min(1, 'Resource name is required')
    ...
    property: z
      ...
      .optional()
      .describe(
        'Property name to check (e.g., "enable_malicious_user_detection"). If omitted, returns all advanced features for the resource.',
      ),
    response_format: ResponseFormatSchema,
  })
  .strict();
...
export const ListAddonServicesSchema = z
  .object({
    tier: z.enum(['STANDARD', 'ADVANCED', 'PREMIUM']).optional().describe('Filter by subscription tier required'),
    activation_type: z
      .enum(['self', 'managed'])
      .optional()
...
export const CheckAddonActivationSchema = z
  .object({
    addon_service: z
      .string()
...
export const GetAddonWorkflowSchema = z
  .object({
    addon_service: z
      .string()
      .min(1, 'Addon service name is required')
