...
// Package root and optional data directory override for development
const PACKAGE_ROOT = join(__dirname, '..', '..');
const DATA_DIR = process.env.MCP_DATA_DIR;
const BUNDLED_DOCS = DATA_DIR ? join(DATA_DIR, 'docs') : join(PACKAGE_ROOT, 'dist', 'docs');

function getDocsRoot(): string {
  return BUNDLED_DOCS;
}

// Subscription metadata path
const BUNDLED_SUBSCRIPTION_METADATA = DATA_DIR
  ? join(DATA_DIR, 'subscription-tiers.json')
  : join(PACKAGE_ROOT, 'dist', 'subscription-tiers.json');
...
function loadSubscriptionMetadata(): SubscriptionMetadata | null {
  ...
  if (existsSync(BUNDLED_SUBSCRIPTION_METADATA)) {
    try {
      const content = readFileSync(BUNDLED_SUBSCRIPTION_METADATA, 'utf-8');
      subscriptionMetadata = JSON.parse(content) as SubscriptionMetadata;
      return subscriptionMetadata;
    } catch (e) {
      console.error(`Failed to load subscription metadata from ${BUNDLED_SUBSCRIPTION_METADATA}:`, e);
    }
  }
